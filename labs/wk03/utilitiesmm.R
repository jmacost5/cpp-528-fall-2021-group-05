...
#choose the variables from the data frame
choose_var <- function(x){
d <- select(d, tractid, mhmval00, mhmval90, hinc00, hu00, own00, rent00, empclf00, clf00, unemp00, prof00, dpov00, npov00, ag25up00, hs00, col00, pop00.x, nhwht00, nhblk00, hisp00, asian00, cbsa, cbsaname, pop90.x)
...
#Create Gentrification Variables
...
d3 <- 
  ...
  mutate( 
    ...
    p.rent.90 = 100 * rent90/hu90)

return(d3)
}

#Gentrification Grouping Percentile
gent_percent <- function(x){
  d3 <-
  d3 %>%
  group_by( cbsaname ) %>%
  mutate( metro.mhv.pct.90 = ntile( mhv.90, 100 ),
          ...
          metro.pay.rank.90 = ntile( (100 - metro.median.pay.90), 100),
          metro.race.rank.90 = ntile( (100-p.col.edu.90), 100 ) ) %>%
  ungroup() %>%
  mutate( metro.mhv.pct.change = metro.mhv.pct.00 - metro.mhv.pct.90,
          pay.change = metro.median.pay.00 - metro.median.pay.90,
          race.change = p.col.edu.00 - p.col.edu.90,
...
gent_change_var <- function (x){
  d3 <-           
  d3 %>%
  select( c( "tractid", "cbsa", "cbsaname",
             "mhv.00", "mhv.90", "mhv.change","pct.change","p.rent.00", "p.rent.90",
